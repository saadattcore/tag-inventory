    ...
    public static class DataSetExtensions
    {
        ...
        private static List<ReceivedBox> GetReceivedBox(DataTable rbDT)
        {
            ...
            foreach (DataRow row in rbDT.Rows)
            {
                ...
                if (rbDT.Columns.Contains("biShipmentID") && DBNull.Value != row["biShipmentID"])
                {
                    rb.ShipmentID = Convert.ToInt64(row["biShipmentID"]);
                }
        ...
        public static List<IssuedBox> GetIssuedBoxList(DataTable table)
        {
